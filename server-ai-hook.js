...
if (!fs.__marketingAgentHtmlPatched) {
  ...
  fs.readFileSync = function patchedReadFileSync(file, options) {
    ...
    if (fileName.endsWith('index.html') && typeof result === 'string' && !result.includes('/marketing-agent.js')) {
      return result.replace('</body>', '<script src="/marketing-agent.js"></script><script src="/recommendations.js"></script><script src="/explainability.js?v=8084714"></script></body>');
    }
    return result;
  };
}
...
if (!express.application.__marketingAiPatched) {
  ...
  function buildDemoAnswer(question, data) {
    ...
    const { workspace, campaigns, customers, content, reviews } = data;
  ...
  async function callOpenAI(question, history, data) {
    ...
    const response = await fetch('https://api.openai.com/v1/responses', {
      ...
      body: JSON.stringify({ model: DEFAULT_MODEL, instructions: system, input: messages, store: false, max_output_tokens: 900 })
    });
    if (!response.ok) {
