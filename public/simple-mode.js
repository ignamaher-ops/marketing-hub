(() => {
  ...
  function showSimpleDashboard() {
    ...
    panel.id = PANEL_ID;
    panel.innerHTML = `
      <div class="mh-simple-welcome">
        <div style="font-size:12px;text-transform:uppercase;letter-spacing:.08em;color:#5B4CF5;font-weight:800;margin-bottom:7px">Resumen</div>
        <h1>¿Cómo está tu negocio?</h1>
        <p>Te mostramos solamente lo más importante para que sepas qué está pasando y qué conviene hacer.</p>
      </div>
      <div class="mh-simple-grid">
        <div class="mh-simple-card"><div class="mh-simple-label">Ventas</div><div class="mh-simple-value" data-simple-sales>—</div><div class="mh-simple-note">Ventas registradas en tus campañas</div></div>
        <div class="mh-simple-card"><div class="mh-simple-label">Clientes</div><div class="mh-simple-value" data-simple-customers>—</div><div class="mh-simple-note">Personas registradas en Marketing Hub</div></div>
        <div class="mh-simple-card"><div class="mh-simple-label">Publicidad</div><div class="mh-simple-status" data-simple-status>Revisando</div><div class="mh-simple-note" data-simple-ad-note>Analizando el rendimiento de tus campañas.</div></div>
      </div>
      <div class="mh-simple-priority">
        <h2>¿Qué deberías hacer ahora?</h2>
        <div class="mh-simple-priority-item"><strong>Revisar tus campañas</strong><p>Marketing Hub puede comparar cuánto invertiste y cuántas ventas consiguió cada campaña.</p></div>
        <div class="mh-simple-priority-item"><strong>Conocer a tus clientes</strong><p>Revisá quién compra, quién dejó de comprar y a quién conviene volver a contactar.</p></div>
      </div>
      <div class="mh-simple-section-title">¿Qué querés hacer?</div>
      <div class="mh-simple-actions">
        <button class="mh-simple-action" data-simple-ai="¿Cómo están mis ventas?"><strong>¿Cómo están mis ventas?</strong><span>Entendé si las ventas están mejorando o empeorando.</span></button>
        ...
        <button class="mh-simple-action" data-simple-ai="¿Dónde estoy perdiendo dinero?"><strong>¿Dónde estoy perdiendo dinero?</strong><span>Encontrá campañas o acciones que necesitan atención.</span></button>
        <button class="mh-simple-action" data-simple-ai="¿Qué debería hacer esta semana?"><strong>¿Qué debería hacer esta semana?</strong><span>Recibí tus prioridades más importantes.</span></button>
      </div>`;

    // Keep the existing dashboard accessible from the rest of the application.
    container.insertBefore(panel, container.firstChild);
    container.querySelectorAll('[data-mh-original="1"]').forEach(el => el.classList.add('mh-simple-original'));
    panel.querySelectorAll('[data-simple-ai]').forEach(btn => btn.addEventListener('click', () => {
      const ai = document.querySelector('.marketing-agent-button, [data-open-marketing-agent], #marketing-agent-btn');
      if (ai) ai.click();
      setTimeout(() => {
        const input = document.querySelector('.marketing-agent-input, #marketing-agent-input, [data-marketing-agent-input]');
        if (input) { input.value = btn.dataset.simpleAi; input.dispatchEvent(new Event('input', {bubbles:true})); input.focus(); }
      }, 250);
    }));
  }

  function updateSimpleMetrics() {
    const sales = document.querySelector('[data-simple-sales]');
    const customers = document.querySelector('[data-simple-customers]');
    if (!sales || !customers) return;
    const campaignRows = Array.from(document.querySelectorAll('table tbody tr'));
    let saleCount = 0;
    campaignRows.forEach(row => {
      const text = row.textContent || '';
      const match = text.match(/\b(\d+)\s*ventas?\b/i);
      if (match) saleCount += Number(match[1]);
    });
  ...
  function init() { injectStyles(); setTimeout(() => { showSimpleDashboard(); updateSimpleMetrics(); }, 500); }
