...
function text(value, max = 160) { const valueText = String(value ?? '').trim(); return valueText.length <= max ? valueText : null; }
function email(value) { const normalized = String(value ?? '').trim().toLowerCase(); return /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(normalized) && normalized.length <= 254 ? normalized : null; }
function oneOf(value, allowed, fallback = null) { const normalized = String(value ?? '').trim().toLowerCase(); return allowed.includes(normalized) ? normalized : fallback; }
...
app.post('/api/auth/register', authLimiter, asyncRoute(async (req, res) => {
  const body = req.body || {};
  const name = text(body.name, 80), normalizedEmail = email(body.email), businessName = text(body.businessName, 120), category = text(body.category || 'Negocio', 60);
  const password = String(body.password || '');
  if (!name || !normalizedEmail || !password || !businessName) return res.status(400).json({ error: 'Completá nombre, email, contraseña y nombre del negocio.' });
  if (password.length < 10 || password.length > 128) return res.status(400).json({ error: 'La contraseña debe tener entre 10 y 128 caracteres.' });
  const client = await pool.connect();
  try {
    await client.query('BEGIN');
    if ((await client.query('SELECT 1 FROM users WHERE email = $1', [normalizedEmail])).rowCount) { await client.query('ROLLBACK'); return res.status(409).json({ error: 'Ese email ya está registrado.' }); }
    const passwordHash = await bcrypt.hash(password, 12);
    const user = (await client.query('INSERT INTO users(email,password_hash,name) VALUES($1,$2,$3) RETURNING id,email,name', [normalizedEmail, passwordHash, name])).rows[0];
    const business = (await client.query('INSERT INTO businesses(name,category) VALUES($1,$2) RETURNING id,name,category', [businessName, category])).rows[0];
...
app.post('/api/auth/login', authLimiter, asyncRoute(async (req, res) => {
  const normalizedEmail = email(req.body?.email), password = String(req.body?.password || '');
  if (!normalizedEmail || !password) return res.status(400).json({ error: 'Ingresá un email y una contraseña válidos.' });
  const user = (await pool.query('SELECT id,email,name,password_hash FROM users WHERE email=$1', [normalizedEmail])).rows[0];
  if (!user || !(await bcrypt.compare(password, user.password_hash))) return res.status(401).json({ error: 'Email o contraseña incorrectos.' });
...
app.patch('/api/workspace', requireAuth, asyncRoute(async (req, res) => {
  ...
  for (const key of allowed) if (Object.prototype.hasOwnProperty.call(req.body || {}, key)) {
    ...
    if (['name','category','address','phone','instagram'].includes(key)) { value=text(value, key === 'address' ? 180 : 120); if (value === null) return res.status(400).json({ error:`${key} is too long` }); }
    if (key==='monthly_budget') value=parseNonNegativeNumber(value);
    if (key==='monthly_budget' && value===null) return res.status(400).json({ error:'monthly_budget must be a non-negative number' });
    updates.push(`${key}=$${values.length+1}`); values.push(value);
  }
...
app.get('/api/dashboard', requireAuth, asyncRoute(async (req,res)=>{
  ...
  const [campaigns, customers, content, reviews, promotions] = await Promise.all([
    pool.query(`SELECT COALESCE(SUM(budget),0) AS budget, COALESCE(SUM(spent),0) AS spent, COALESCE(SUM(leads),0) AS leads, COALESCE(SUM(sales),0) AS sales FROM campaigns WHERE business_id=$1`,[id]),
    pool.query(`SELECT COUNT(*)::int AS total, COUNT(*) FILTER (WHERE created_at >= NOW() - INTERVAL '30 days')::int AS new_last_30 FROM customers WHERE business_id=$1`,[id]),
    pool.query(`SELECT COUNT(*)::int AS total, COUNT(*) FILTER (WHERE status='published')::int AS published FROM content WHERE business_id=$1`,[id]),
    pool.query(`SELECT COUNT(*)::int AS total, COALESCE(AVG(rating),0) AS average_rating, COUNT(*) FILTER (WHERE status='pending')::int AS pending FROM reviews WHERE business_id=$1`,[id]),
    pool.query(`SELECT COUNT(*)::int AS total, COUNT(*) FILTER (WHERE status='active')::int AS active FROM promotions WHERE business_id=$1`,[id])
  ]);
  const c=campaigns.rows[0], cu=customers.rows[0], co=content.rows[0], r=reviews.rows[0], p=promotions.rows[0];
  const budget=Number(c.budget)||0, spent=Number(c.spent)||0;
  res.json({ metrics:{campaignBudget:budget,campaignSpent:spent,remainingBudget:Math.max(0,budget-spent),leads:Number(c.leads)||0,sales:Number(c.sales)||0,customers:Number(cu.total)||0,newCustomers:Number(cu.new_last_30)||0,content:Number(co.total)||0,publishedContent:Number(co.published)||0,reviews:Number(r.total)||0,averageRating:Number(r.average_rating)||0,pendingReviews:Number(r.pending)||0,promotions:Number(p.total)||0,activePromotions:Number(p.active)||0} });
}));
...
app.post('/api/customers', requireAuth, asyncRoute(async(req,res)=>{const id=await workspaceId(req);if(!id)return res.status(404).json({error:'Workspace not found'});const b=req.body||{},name=text(b.name,120);if(!name)return res.status(400).json({error:'name is required'});const customerEmail=b.email?email(b.email):null;if(b.email&&!customerEmail)return res.status(400).json({error:'Invalid email'});const phone=b.phone?text(b.phone,40):null;const status=oneOf(b.status,['new','active','inactive','frequent'],'new');const r=await pool.query(`INSERT INTO customers(business_id,name,phone,email,status,purchases,total_spent,last_purchase) VALUES($1,$2,$3,$4,$5,0,0,$6) RETURNING id,name,phone,email,status,purchases,total_spent,last_purchase,created_at`,[id,name,phone,customerEmail,status,b.last_purchase||new Date().toISOString().slice(0,10)]);res.status(201).json({customer:r.rows[0]});}));
app.patch('/api/customers/:id',requireAuth,asyncRoute(async(req,res)=>{const id=await workspaceId(req);if(!id)return res.status(404).json({error:'Workspace not found'});const allowed=['name','phone','email','status','purchases','total_spent','last_purchase'],u=[],v=[];for(const key of allowed)if(Object.prototype.hasOwnProperty.call(req.body||{},key)){let x=req.body[key];if(key==='name'){x=text(x,120);if(x===null)return res.status(400).json({error:'name is too long'});}if(key==='phone'){x=x?text(x,40):null;if(x===null)return res.status(400).json({error:'phone is too long'});}if(key==='email'){x=x?email(x):null;if(req.body[key]&&x===null)return res.status(400).json({error:'Invalid email'});}if(key==='status'){x=oneOf(x,['new','active','inactive','frequent']);if(!x)return res.status(400).json({error:'Invalid customer status'});}if(key==='purchases'){x=parseNonNegativeInt(x);if(x===null)return res.status(400).json({error:'purchases must be a non-negative integer'});}if(key==='total_spent'){x=parseNonNegativeNumber(x);if(x===null)return res.status(400).json({error:'total_spent must be a non-negative number'});}u.push(`${key}=$${v.length+1}`);v.push(x);}if(!u.length)return res.status(400).json({error:'No fields to update'});v.push(id,req.params.id);const r=await pool.query(`UPDATE customers SET ${u.join(',')} WHERE business_id=$${v.length-1} AND id=$${v.length} RETURNING id,name,phone,email,status,purchases,total_spent,last_purchase,created_at`,v);if(!r.rowCount)return res.status(404).json({error:'Customer not found'});res.json({customer:r.rows[0]});}));
...
app.post('/api/content',requireAuth,asyncRoute(async(req,res)=>{const id=await workspaceId(req);if(!id)return res.status(404).json({error:'Workspace not found'});const b=req.body||{},title=text(b.title,160),type=text(b.type||b.format||'Post',40),platform=text(b.platform,40),status=oneOf(b.status,['draft','scheduled','published'],'draft');if(!title||!platform)return res.status(400).json({error:'title and platform are required'});const copy=b.copy?text(b.copy,5000):null;if(b.copy&&copy===null)return res.status(400).json({error:'copy is too long'});const r=await pool.query(`INSERT INTO content(business_id,title,type,platform,scheduled_date,status,copy) VALUES($1,$2,$3,$4,$5,$6,$7) RETURNING id,title,type,platform,scheduled_date,status,copy,created_at`,[id,title,type,platform,b.scheduled_date||b.date||null,status,copy]);res.status(201).json({content:r.rows[0]});}));
app.patch('/api/content/:id',requireAuth,asyncRoute(async(req,res)=>{const id=await workspaceId(req);if(!id)return res.status(404).json({error:'Workspace not found'});const allowed=['title','type','platform','scheduled_date','status','copy'],u=[],v=[];for(const key of allowed)if(Object.prototype.hasOwnProperty.call(req.body||{},key)){let x=req.body[key];if(['title','type','platform'].includes(key)){x=text(x,key==='title'?160:40);if(x===null)return res.status(400).json({error:`${key} is too long`});}if(key==='copy'){x=x===null?'':text(x,5000);if(x===null)return res.status(400).json({error:'copy is too long'});}if(key==='status'){x=oneOf(x,['draft','scheduled','published']);if(!x)return res.status(400).json({error:'Invalid content status'});}u.push(`${key}=$${v.length+1}`);v.push(x);}if(!u.length)return res.status(400).json({error:'No fields to update'});v.push(id,req.params.id);const r=await pool.query(`UPDATE content SET ${u.join(',')} WHERE business_id=$${v.length-1} AND id=$${v.length} RETURNING id,title,type,platform,scheduled_date,status,copy,created_at`,v);if(!r.rowCount)return res.status(404).json({error:'Content not found'});res.json({content:r.rows[0]});}));
...
app.post('/api/promotions',requireAuth,asyncRoute(async(req,res)=>{const id=await workspaceId(req);if(!id)return res.status(404).json({error:'Workspace not found'});const b=req.body||{},title=text(b.title||b.name,160),code=text(b.code,40)?.toUpperCase(),discount=text(b.discount,80),status=oneOf(b.status,['active','inactive'],'active');if(!title||!code||!discount)return res.status(400).json({error:'title, code and discount are required'});const r=await pool.query(`INSERT INTO promotions(business_id,title,code,discount,start_date,end_date,uses,status) VALUES($1,$2,$3,$4,$5,$6,0,$7) RETURNING id,title,code,discount,start_date,end_date,uses,status,created_at`,[id,title,code,discount,b.start_date||b.start||null,b.end_date||b.end||null,status]);res.status(201).json({promotion:r.rows[0]});}));
